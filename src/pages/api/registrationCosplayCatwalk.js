...
CREATE TABLE registration_showact (
    ...
    catwalk_conditions BOOLEAN NOT NULL,
...
const connection = mysql.createPool({
  ...
  if (errors.length > 0) {
    ...
    await logError(clientIp, "Catwalk Anmeldung", email, errorlog);
  ...
  `;
    ...
    const uploadDir = path.join(process.cwd(), `/private/catwalk`);
    ...
    for (const file of file) {
      ...
      const publicPath = `/catwalk/${filename}`;
